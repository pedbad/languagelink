...
from django.utils.text import Truncator  # (kept; used elsewhere)
...
def display_name(user) -> str:
    """
    Prefer 'First Last'; otherwise fall back to email; otherwise 'A student'.
    """
    first = (getattr(user, "first_name", "") or "").strip()
    last = (getattr(user, "last_name", "") or "").strip()
    full = f"{first} {last}".strip()
    return full or getattr(user, "email", "A student")
...
def notify_booking_created(booking):
    ...
    def _send():
        # Student confirmation — NOW includes advisor full name (and retains email)
        send_plain_email(
            subject="Booking confirmed",
            to=[student.email],
            body_text=(
                f"Hello {getattr(student, 'first_name', '') or 'there'},\n\n"
                f"Your booking with {display_name(teacher)} ({getattr(teacher, 'email', 'the teacher')}) is confirmed.\n"
                f"When: {when}\n"
                f"{msg_for_student}"
        ...
        # Teacher notification — NOW shows student full name + email
        send_plain_email(
            subject="New booking received",
            to=[teacher.email],
            body_text=(
                f"Hi {getattr(teacher, 'first_name', '') or 'there'},\n\n"
                f"{display_name(student)} <{student.email}> booked a slot.\n"
                f"When: {when}\n"
                f"{msg_for_teacher}"
        ...
        # Admin copy — show name + email (unchanged from prior improvement)
        admins = admin_emails()
        if admins:
            send_plain_email(
                ...
                body_text=(
                    ...
                    f"Student: {display_name(student)} <{student.email}>\n"
                    f"When: {when}\n"
                    f"{msg_for_admin}"
...
def notify_resource_note_created(note):
    ...
    """
    from django.urls import reverse  # safe to re-import
